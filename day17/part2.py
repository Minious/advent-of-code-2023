    ...
    for dir in DIRS:
        ...
        if vert_from[2] != opposite_dir:
            ...
            if to[0] >= 0 and to[1] >= 0 and to[0] < len(grid[0]) and to[1] < len(grid):
                if dir != vert_from[2] and vert_from[3] + 1 >= 4:
                    if to == (len(grid[0]) - 1, len(grid) - 1):
                        pass
                    else:
                        neighbors.append((to[0], to[1], dir, 0))
                elif dir == vert_from[2] and vert_from[3] + 1 < 10:
                    if to == (len(grid[0]) - 1, len(grid) - 1):
                        if vert_from[3] + 1 >= 3:
                            neighbors.append(to)
                    else:
...
def get_reverse_graph(graph):
    _graph = {v: {} for v in graph}
    for v in graph:
        for n in graph[v]:
            if n not in _graph:
                _graph[n] = {}
            _graph[n][v] = graph[v][n]
    return _graph
...
if __name__ == '__main__':
    ...
    dist, prev = djikstra_minheap(graph, (0, 0))
